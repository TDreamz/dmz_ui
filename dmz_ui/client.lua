...
RegisterNetEvent("dmz_ui:SendNotify")
AddEventHandler("dmz_ui:SendNotify", function(type, item, options)
...
function SendNotify(type, item, override_options)
	...
	options.inventory =  Config.InventoryImage
	options.timeout = Config.DisplayDelay * 700
	options.type = type
	if item.name ~= nil and item.label ~= nil then
		SendNUIMessage({type = 'notify', item = item, options = options})
	else
		print('[dmz_ui] = > Item Error!')
	end
end
	...
	for i=1, #currentAccounts, 1 do
		...
		if  currentAccounts[i].name == 'black_money' and not isDropBlackMoney	 then
			if currentAccounts[i].money > account.money then
				...
				TriggerEvent("dmz_ui:SendNotify", 'remove',  {
					name = 'black_money	',
				...
				TriggerEvent("dmz_ui:SendNotify", 'add',  {
					name = 'black_money	',
					label = (Config.BlackMoneyLabel):format(ESX.Math.GroupDigits(ESX.Math.Round(money)))
				})
			end
...
AddEventHandler('_esx:useItem', function(item)
	--print('useItem')
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("dmz_ui:SendNotify", 'use', item)
	end
end)
...
AddEventHandler('_esx:dropItem', function(item)
	...
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("dmz_ui:SendNotify", 'remove', item, {action = 'ทิ้ง'})
	end
end)
...
AddEventHandler('esx:addInventoryItem', function(item, count)
	...
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("dmz_ui:SendNotify", 'add', item)
	end
end)
...
AddEventHandler('_esx:removeInventoryItem', function(item, count)
	...
	if item.name ~= nil and item.label ~= nil then
		TriggerEvent("dmz_ui:SendNotify", 'remove', item)
	end
end)
